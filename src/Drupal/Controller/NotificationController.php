...
class  NotificationController extends ControllerBase{
    ...
    public function __construct()
    {
        $this->notificationService = new NotificationService(
            ...
            getenv('MS_NOTIFICATION_URL')
        );
    }

    public function getNotifications()
    {
        $response = $this->notificationService->getNotifications($this->currentUser->id());
        if(isset($response['try']))
        {
            throw new Exception($response['message']);
        }
        return new JsonResponse(json_decode($response->getBody()->getContents(), true), $response['code']);
    }
